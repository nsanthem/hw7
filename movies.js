  ...
  if (user) {
    ...
    document.querySelector('.signed-in-user').innerHTML = `
      <strong class="text-center mt-4 px-4 py-2 rounded">Signed in as ${user.displayName}</strong>
    ` 
  ...
  for (let i=0; i<movies.length; i++) {
    ...
    document.querySelector(`.movie-${movie.id}`).addEventListener('click', async function(event) {
      ...
      console.log(`movie ${movie.id} watch button clicked!`)
